...
export const MapPicker = component$<MapPickerProps>(({ initialLat = -7.9666, initialLng = 112.6326, onLocationSelect$, currentValue }) => {
  ...
  useVisibleTask$(({ track }) => {
    ...
    if (currentValue && currentValue.includes(",")) {
      const [lat, lng] = currentValue.split(",").map((v) => parseFloat(v.trim()));
  ...
  useVisibleTask$(async ({ track, cleanup }) => {
    ...
    // Initialize map
    const map = L.map(mapContainer.value).setView([selectedLat.value, selectedLng.value], 13);
  ...
  return (
    <>
      <button type="button" class="btn btn-outline btn-sm gap-2" onClick$={() => (isOpen.value = true)}>
        ...
        <div class="modal modal-open">
          <div class="modal-box max-w-4xl">
            <div class="flex items-center justify-between mb-4">
              ...
              <button type="button" class="btn btn-sm btn-circle btn-ghost" onClick$={() => (isOpen.value = false)}>
                <LuX class="w-5 h-5" />
              </button>
            </div>
            ...
            <div class="alert alert-info mb-4">
              ...
              <span class="text-sm">
                Klik pada peta atau seret marker untuk memilih lokasi. Lokasi awal akan menggunakan posisi Anda saat ini jika memungkinkan.
              </span>
            </div>

            <div ref={mapContainer} style="height: 400px; width: 100%; border-radius: 8px;" class="mb-4" />
            ...
            <div class="modal-action">
              <button type="button" class="btn btn-ghost" onClick$={() => (isOpen.value = false)}>
                Batal
              </button>
