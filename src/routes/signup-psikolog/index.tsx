...
export default component$(() => {
  ...
  const handleSubmit = $(async (e: Event) => {
    ...
    console.log('📝 Signup Psikolog - Form Data:', form.value);
    
    try {
      const response = await api.post('/auth/signup/psikolog', { ...form.value });
      console.log('🎉 Signup Psikolog - Success Response:', response.data);
      success.value = 'Pendaftaran berhasil!';
      form.value = { name: '', email: '', password: '', no_telp: '', spesialis: '', lokasi: '' };
    } catch (err: any) {
  ...
  return (
    <div class="container mx-auto max-w-md p-4">
      ...
      <form preventdefault:submit onSubmit$={handleSubmit} class="flex flex-col gap-3">
        ...
        <input class="input input-bordered" name="spesialis" placeholder="Spesialis" value={form.value.spesialis} onInput$={handleInput} />
